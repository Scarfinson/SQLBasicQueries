...
SELECT EMPLOYEES.FIRST_NAME, EMPLOYEES.SALARY FROM EMPLOYEES INNER JOIN DEPARTMENTS ON EMPLOYEES.DEPARTMENT_ID = DEPARTMENTS.DEPARTMENT_ID AND DEPARTMENT_NAME LIKE 'Finance' WHERE EMPLOYEES.SALARY >= 13000;

SELECT FIRST_NAME, EMAIL FROM EMPLOYEES WHERE SALARY>1000 AND SALARY <= 10000 AND EMAIL LIKE 'A%';

SELECT EMPLOYEES.FIRST_NAME, EMPLOYEES.SALARY, DEPARTMENTS.DEPARTMENT_NAME, LOCATIONS.CITY FROM EMPLOYEES INNER JOIN DEPARTMENTS ON EMPLOYEES.DEPARTMENT_ID = DEPARTMENTS.DEPARTMENT_ID INNER JOIN LOCATIONS ON DEPARTMENTS.LOCATION_ID = LOCATIONS.LOCATION_ID WHERE EMPLOYEES.FIRST_NAME LIKE 'S%' AND LOCATIONS.CITY LIKE 'Seattle';

SELECT LAST_NAME, SALARY, (SALARY*0.10)AS "BONO PRODUCCIÓN" FROM EMPLOYEES WHERE COMMISSION_PCT IS NULL AND SALARY >9000 ORDER BY 2 DESC;

SELECT EMPLOYEES.FIRST_NAME, EMPLOYEES.SALARY, LOCATIONS.CITY, DEPARTMENTS.DEPARTMENT_NAME FROM EMPLOYEES INNER JOIN DEPARTMENTS ON EMPLOYEES.DEPARTMENT_ID = DEPARTMENTS.DEPARTMENT_ID INNER JOIN LOCATIONS ON DEPARTMENTS.LOCATION_ID = LOCATIONS.LOCATION_ID WHERE LOCATIONS.CITY LIKE 'Toronto' ORDER BY 1 ASC;

SELECT AVG(SALARY) FROM EMPLOYEES WHERE LAST_NAME LIKE 'M%';

SELECT EMPLOYEES.FIRST_NAME, DEPARTMENTS.DEPARTMENT_NAME, EMPLOYEES.SALARY, COUNTRIES.COUNTRY_NAME, LOCATIONS.CITY FROM EMPLOYEES INNER JOIN DEPARTMENTS ON EMPLOYEES.DEPARTMENT_ID = DEPARTMENTS.DEPARTMENT_ID INNER JOIN LOCATIONS ON DEPARTMENTS.LOCATION_ID = LOCATIONS.LOCATION_ID INNER JOIN COUNTRIES ON LOCATIONS.COUNTRY_ID = COUNTRIES.COUNTRY_ID INNER JOIN JOBS ON EMPLOYEES.JOB_ID = JOBS.JOB_ID WHERE JOBS.JOB_TITLE LIKE 'Programmer';

SELECT COUNT(EMPLOYEE_ID) AS "Trabajadores_ST_MAN" FROM EMPLOYEES WHERE JOB_ID LIKE 'ST_MAN';
